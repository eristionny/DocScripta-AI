...
def obter_tema():
    ...
    return st.session_state.get("tema_docscripta", TEMA_CLARO)


def label_tema(texto):
    """Exibe label com cor adaptada ao tema."""
    tema = obter_tema()
    cor = "#1a1a2e" if tema == TEMA_CLARO else "#e0e0e0"
    st.markdown(
        f'<p style="color:{cor};font-weight:600;margin-bottom:4px;">'
        f'{texto}</p>',
...
def subheader_tema(texto):
    """Exibe subheader com cor adaptada ao tema."""
    tema = obter_tema()
    cor = "#1a1a2e" if tema == TEMA_CLARO else "#e0e0e0"
    st.markdown(
        f'<h3 style="color:{cor};">{texto}</h3>',
        unsafe_allow_html=True
    )


# ============================================================
# CSS — TEMA CLARO / ESCURO
# ============================================================

def aplicar_css_tema():
    """Aplica o CSS completo para claro ou escuro."""
    tema = obter_tema()
    ...
    if tema == TEMA_ESCURO:
        ...
        /* === TABS === */
        .stTabs [data-testid="stTabNav"] button {
            color: #aaaacc !important;
        }
        .stTabs [data-testid="stTabNav"] button[data-baseline=true] {
        ...
        css = """
        <style>
        /* Tema claro — padrão do Streamlit */
        .stApp {
...
def injetar_pwa():
    """Insere links de PWA manifest e service worker."""
    pwa_html = """
...
# ============================================================
# TONNYBOT — AVATAR CLICÁVEL (ALTERNA TEMA)
# ============================================================

def exibir_tonnybot():
    """Exibe o avatar do TonnyBot como elemento clicável."""
    tema = obter_tema()
    tema_oposto = TEMA_ESCURO if tema == TEMA_CLARO else TEMA_CLARO
    ...
    <div style="
        ...
        cursor:pointer;
    " onclick="""
    tonnybot_html += f"window.parent.location.href='?tema={tema_oposto}'"
    tonnybot_html += f'">'
    tonnybot_html += """
        <div style="
            width:56px;
            height:56px;
            border-radius:50%;
            background:linear-gradient(135deg, #4a90d9, #1a1a2e);
            display:flex;
            align-items:center;
            justify-content:center;
            font-size:30px;
            box-shadow: 0 3px 12px rgba(26,26,46,0.4);
        ...
        <div>
            <div style="font-weight:700;font-size:18px;color:#4a90d9;">
                TonnyBot
            </div>
    ...
    st.markdown(tonnybot_html, unsafe_allow_html=True)


# ============================================================
# DETECTAR TEMA VIA URL
# ============================================================

def detectar_tema_url():
    """Detecta parâmetro ?tema= na URL para alternar."""
    import urllib.parse

    try:
        params = st.query_params
        if "tema" in params:
            tema_param = params["tema"]
            if tema_param in [TEMA_CLARO, TEMA_ESCURO]:
                st.session_state["tema_docscripta"] = tema_param
                # Remove o parâmetro da URL para não ficar persistindo
                del params["tema"]
...
if "tema_docscripta" not in st.session_state:
    st.session_state["tema_docscripta"] = TEMA_CLARO
...
with st.sidebar:
    ...
    tema_selecionado = st.radio(
        ...
        format_func=lambda x: (
            "☀️ Claro" if x == TEMA_CLARO
            else "🌙 Escuro"
        ),
        key="radio_tema"
    )

    if tema_selecionado != st.session_state["tema_docscripta"]:
        st.session_state["tema_docscripta"] = tema_selecionado
        st.rerun()

    st.markdown("---")

    st.caption("DocScripta AI v2.0")
    st.caption("Assistente acadêmico para")
    st.caption("pesquisa, revisão e análise.")
...
with aba_principal:
    ...
    # --------------------------------------------------------
    # 🎤 MICROFONE — FORA DO FORM!
    # componente_microfone() usa st.components.v1.html
    # e NÃO pode ficar dentro de st.form().
    # Chamado UMA ÚNICA VEZ, ANTES do formulário.
    # --------------------------------------------------------
    ...
    with st.form(
        ...
        prompt_texto = st.text_area(
            ...
            key=chave_campo,
            placeholder=(
                "Digite sua pergunta ou use o 🎤 acima..."
            ),
            height=100
        )
